...
int main()
{
	...
	printf("\nВыберите функцию:\n1.sin(x)\n2.cos(x)\n3.e^x\n4.csc(x) x не должен быть кратен числу 'пи'\n");
	do
	{
		flag = 0;
		scanf("%i", &func);
		...
		do
		{
			...
			if ((func == 1) || (func == 2) || (func == 4))
			{
				...
				if (func == 4)
				{
					...
					{
						flag = 0;
						printf("cosec не может принимать значение pi*K. Введите заново!");
					}
				}
			}
		} while (flag == 0);
